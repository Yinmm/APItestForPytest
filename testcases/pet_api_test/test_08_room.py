...
class TestRoom(object):
    ...
    @allure.story("用例--获取装修信息")
    @allure.description("该用例是获取装修信息的测试")
    @pytest.mark.single
    @pytest.mark.smoke
    def test_room_info(self, pet_login_hasrole_fixture):
        logger.info("*************** 开始执行用例 ***************")
        pet_info = pet_login_hasrole_fixture
        token = pet_info["data"]["token"]
        result = room_get(token)
